    ...
    <div style={{ minHeight: "100vh", background: "#f3f6fb", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 40, width: "100%", maxWidth: 440, boxShadow: "0 2px 16px rgba(0,0,0,0.08)" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: "bold", marginBottom: 8 }}>プロフィールを設定しましょう</h2>
          <p style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.7 }}>
            練習中にお名前でお呼びします。<br />呼び名は本名でなくても大丈夫です。
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Field label="お名前（呼び名）" required>
            <input
              ...
              style={inputStyle}
            />
            <p style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>※ 20文字以内・本名でなくてもOK</p>
          </Field>
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: "100%", padding: 12, background: "linear-gradient(180deg, #2563eb, #1d4ed8)", color: "#fff", border: "none", borderRadius: 8, fontWeight: "bold", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, fontSize: 16, marginTop: 8 }}
          >
            {loading ? "保存中..." : "練習を始める →"}
          </button>
        </form>
